...
//! `#[ignore]`, and BULLET_LIVE_PROVIDERS containing `claude`.
#![cfg(feature = "live")]
...
fn enabled() -> bool {
    std::env::var("BULLET_LIVE_PROVIDERS").is_ok_and(|v| v.split(',').any(|p| p.trim() == "claude"))
}
...
async fn live_smoke_claude() {
    if !enabled() {
        eprintln!("live_smoke_claude: skipped (BULLET_LIVE_PROVIDERS)");
        return;
    }
    let adapter = ClaudeAdapter::new();
    let profile = ProfileRef {
        profile_id: bullet_domain::ProfileId::from_seed("live-claude"),
    ...
    let handle = adapter
        .start(StartSession {
            session_id: AgentSessionId::new(synthetic_uuid("live-claude")),
    ...
    eprintln!(
        "RECEIPT provider=claude version={} session={} usage={} wall_s={} exit={:?}",
        probe.version,
