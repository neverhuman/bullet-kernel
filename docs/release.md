...
| Secret and dependency scan | `bash scripts/ci-local.sh security`: `gitleaks detect`, `cargo deny --locked check licenses advisories bans sources` against the committed `deny.toml`, `zizmor .`, and an independent freshness proof of the RustSec advisory database | runs and fails closed; it produces no release artifact |
