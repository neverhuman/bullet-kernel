...
| --- | ---: | --- |
| standalone | 1257 | all component tests outside the provider-contract/simulation, egress, and family partitions; every selected identity executes with zero skipped, including the explicitly feature-enabled verifier fixture tests |
...
| total | 1303 | exact union of the four disjoint partitions above |
...
enumeration alone is not a successful test execution. The September 11
closeout inventory, hashed from a real nextest JSON, adds six standalone
identities (v2 binding credential forward, ledger D1, HTTP then real-runner
stub D1, hex-safe TUI paint, Codex argv, Cursor typed empty-stream refusal).
...
| Lane | Scope |
| --- | --- |
| `fast` | exactly 1257 standalone nextest identities, all executed with zero skipped |
