...
| security | `just security` | gitleaks (no-git) plus `cargo deny check bans`; a missing tool fails | hygiene gate; no evidence class |
...
`security`, each by calling the same `ops/ci/<lane>.sh`. `audit`, `egress`, and
`nightly` are local lanes with no hosted job. Hosted runners provision pinned
`cargo-nextest`, `gitleaks`, and `cargo-deny`; `jankurai` and the egress tools
are local-only.
